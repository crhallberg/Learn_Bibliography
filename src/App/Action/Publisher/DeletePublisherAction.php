        ...
        if ($request->getMethod() == "POST") {
            ...
            if(isset($post['submitt'])) {
                if ($post['submitt'] == "Cancel") {
                    ...
                    return new HtmlResponse($this->template->render('app::manage_publisher', ['rows' => $paginator,'previous' => $this->previous,'next' => $this->next,'request' => $request]));
                }
                ...
                if ($post['submitt'] == "Delete") {
                    ...
                    if(!is_null($post['id'])) {
                        ...
                        return new HtmlResponse($this->template->render('app::manage_publisher', ['rows' => $paginator,
                        'previous' => $this->previous,'next' => $this->next, 'countp' => $countPages, 'request' => $request]));
                    }
                    // echo $_POST['id'];
                }
            }
        }
        return new HtmlResponse($this->template->render('app::delete_publisher', ['rows' => $rows, 'request' => $request]));
    }
